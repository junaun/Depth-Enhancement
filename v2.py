...
def main():
    ...
    for folder in scan_dir():
        ...
        chunks = chunk_list(mech_poses[:10], num_processes)
        e = 0
        for t_pro in chunks: # Multiprocessing
            ...
            for u,trans in enumerate(t_pro): 
                ...
                source = pcd1.voxel_down_sample(voxel_size = 0.01)
                target = pcd2.voxel_down_sample(voxel_size = 0.01)
                tf_param, _, _ = cpd.registration_cpd(source, target, maxiter = 2000, tol = 0.001, update_scale =False, tf_type_name = "rigid")
                # tf_param, _, _ = filterreg.registration_filterreg(source, target, target.normals, objective_type = "pt2pl", sigma2=1)
                ...
                pcd1.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.02, max_nn=30))
                pcd2.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.02, max_nn=30))
                ...
                # o3d.visualization.draw_geometries([pcd1, pcd2])
                process_1 = Process(target=point_cloud_to_rgb_image, args=(pcd1, real_intrinsic_value, i))
                processs_1.append(process_1)
                process_2 = Process(target=point_cloud_to_depth_image, args=(pcd1, real_intrinsic_value, i))
...
if __name__ == "__main__":
    ...
    # box = o3d.geometry.AxisAlignedBoundingBox([-0.13, 0.37, -0.05], [0.25, 0.60, 0.20])
